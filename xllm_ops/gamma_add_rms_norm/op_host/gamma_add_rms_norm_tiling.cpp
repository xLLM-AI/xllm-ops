...
platform_ascendc::SocVersion gammaAddRmsNormSocVersion;
...
uint8_t getPerformanceFlag(uint32_t num_col, gert::Shape x_shape, gert::Shape gamma_shape, uint32_t xDtypeKey)
{
    ...
    if(gammaAddRmsNormSocVersion != platform_ascendc::SocVersion::ASCEND910B) {
...
static bool IsRegbaseSocVersion(const gert::TilingContext* context)
{
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(context->GetPlatformInfo());
    const auto npuArch = ascendcPlatform.GetCurNpuArch();
    return npuArch == NpuArch::DAV_3510 || npuArch == NpuArch::DAV_5102;
}
    ...
    for (uint32_t i = 0; i < x1DimNum; i++) {
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                context->GetNodeName(), "x1", x1_shape->GetStorageShape().ToString().c_str(),
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                ...
                (x2_shape->GetStorageShape().ToString() + " and " +
                 x1_shape->GetStorageShape().ToString()).c_str(),
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                ...
                (y_shape->GetStorageShape().ToString() + " and " +
                 x1_shape->GetStorageShape().ToString()).c_str(),
        ...
        if (norm_key == RMS_NORM_KEY || norm_key == PRE_RMS_NORM) {
            OP_CHECK_IF(
                ...
                OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                    ...
                    (x_shape->GetStorageShape().ToString() + " and " +
                     x1_shape->GetStorageShape().ToString()).c_str(),
        ...
        for (uint32_t i = 0; i < x1DimNum - gammaDimNum; i++) {
            OP_CHECK_IF(
                ...
                OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                    ...
                    (rstd_shape->GetStorageShape().ToString() + " and " +
                     x1_shape->GetStorageShape().ToString()).c_str(),
        ...
        for (uint32_t i = 0; i < gammaDimNum; i++) {
            OP_CHECK_IF(
                ...
                OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                    ...
                    (gamma_shape->GetStorageShape().ToString() + " and " +
                     x1_shape->GetStorageShape().ToString()).c_str(),
            ...
            OP_CHECK_IF(
                ...
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                    ...
                    rstd_shape->GetStorageShape().ToString().c_str(),
    ...
    gert::TilingContext* context, uint32_t& numCore, uint64_t& ubSize)
{
    ...
    if (ptrCompileInfo == nullptr) {
        ...
        gammaAddRmsNormSocVersion = ascendc_platform.GetSocVersion();
        ...
        gammaAddRmsNormSocVersion = ptrCompileInfo->socVersion;
    }
    ubSize -= UB_USED;
}
    ...
    uint32_t dataPerBlock, uint32_t numColAlign, uint32_t& modeKey, uint32_t isPerformance)
{
    ...
    } else if (blockFactor == 1 && gammaAddRmsNormSocVersion != platform_ascendc::SocVersion::ASCEND310P) {
        modeKey = MODE_SINGLE_N;
    } else if (numColAlign <= SMALL_REDUCE_NUM && gammaAddRmsNormSocVersion != platform_ascendc::SocVersion::ASCEND310P) {
