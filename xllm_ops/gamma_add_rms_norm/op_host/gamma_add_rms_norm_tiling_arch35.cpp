...
/*!
 * \file gamma_add_rms_norm_tiling_arch35.cpp
 * \brief
 */
#include <iostream>
...
{
    // queBufSize: UB space required for data movement.
    uint32_t queBufSize = bufferNum * length * dtypeSize * QUE_NUM + FLOAT_PER_REAPEAT * bufferNum * FLOAT_BYTE_SIZE;
    uint32_t tmpBufSzie = 0; // tmpBufSzie: temporary UB space required for computation.
    if (split) {
        // Split-D case.
        tmpBufSzie = (dtype == ge::DT_FLOAT) ? 0 : length * FLOAT_BYTE_SIZE * NUM_2;
    } else {
        // Normal case: float16 and bfloat16 require an additional buffer for conversion to FP32.
...
ge::graphStatus TilingGammaAddRmsNormRegbase(gert::TilingContext* context)
{
    ...
    // Calculate the boundary for binary-tree accumulation.
    ...
    // Number of rows that can be fully loaded into UB.
    ...
    if (rowFactor >= 1) {
        // The reduction dimension can be fully loaded into UB.
        rowFactor = std::min(rowFactor, blockFactor);   // Actual number of rows to load.
