...
/*!
 * \file gamma_add_rms_norm_merge_n.h
 * \brief add rms norm merge n file
 */
#ifndef GAMMA_ADD_RMS_NORM_MERGE_N_H_
#define GAMMA_ADD_RMS_NORM_MERGE_N_H_
...
class KernelGammaAddRmsNormMergeN {
public:
    __aicore__ inline KernelGammaAddRmsNormMergeN(TPipe* pipe)
    {
        Ppipe = pipe;
    }

    __aicore__ inline void InitParams(const GammaAddRMSNormTilingData* tiling)
    {
...
private:
        ...
        LocalTensor<T> xLocal, LocalTensor<T> gammaLocal, LocalTensor<float> rstdLocal, uint32_t calc_row_num, uint32_t elementNum)
    {
        ...
        if constexpr (is_same<T, half>::value) {
            ...
            // Cast BF16 values to FP32 before multiplication.
            LocalTensor<float> sqx = sqxBuf.Get<float>();
            LocalTensor<float> x_fp32 = xFp32Buf.Get<float>();
...
#endif // _GAMMA_ADD_RMS_NORM_MERGE_N_H_
