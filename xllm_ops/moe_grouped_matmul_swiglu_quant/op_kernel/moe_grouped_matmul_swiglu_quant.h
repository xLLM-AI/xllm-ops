 ...
 * \brief
 */
#ifndef MOE_GROUPED_MATMUL_SWIGLU_QUANT_H
#define MOE_GROUPED_MATMUL_SWIGLU_QUANT_H

#include "moe_grouped_matmul_swiglu_quant_utils.h"
namespace MOE_GROUPED_MATMUL_SWIGLU_QUANT {
/** @brief intenal computation class
 */
template <class mmType, bool sync = false, typename CHANNELDTYPE = float>
class MoeGMMSwigluCompute {
public:
    ...
    /** @brief constructor */
    __aicore__ inline MoeGMMSwigluCompute(typename mmType::MT &mm_) : mm(mm_)
    {
    }
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR weight, GM_ADDR perChannelScale, GM_ADDR perTokenScale,
                                ...
                                const MoeGMMSwigluBaseParams *__restrict gmmBaseParamsIN,
                                const TCubeTiling *__restrict mmTilingDataIN, const MoeGMMSwiglu *__restrict gmmSwigluIN,
                                TPipe *tPipeIN);
...
private:
    ...
    __aicore__ inline void UpdateMnConfig(MNConfig &mnConfig, uint32_t groupIdx);
...
private:
    ...
    const MoeGMMSwigluBaseParams *__restrict gmmBaseParams;
    const MoeGMMSwiglu *__restrict gmmSwiglu;
...
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::Init(
    GM_ADDR x, GM_ADDR weight, GM_ADDR perChannelScale, GM_ADDR perTokenScale, GM_ADDR groupList, GM_ADDR quantOutput,
    GM_ADDR quantScaleOutput, GM_ADDR workspace, const MoeGMMSwigluBaseParams *__restrict gmmSwigluBaseParamsIn,
    const TCubeTiling *__restrict mmTilingDataIN, const MoeGMMSwiglu *__restrict gmmSwigluIN, TPipe *tPipeIN)
{
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::Process()
{
    MNConfig mnConfig;
    VecConfig vecConfig;
...
MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::PreLoadTokenAndChannel(LocalTensor<float> &channelScaleLocal,
                                                                     VecConfig &vecConfig)
{
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::MMCompute(uint32_t groupIdx, MNConfig &mnConfig,
                                                                               uint32_t coreIdx)
{
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::UpdateMnConfig(MNConfig &mnConfig, uint32_t groupIdx)
{
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::SetMNConfig(const int32_t splitValue,
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::SetMKN(const int32_t splitValue,
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline uint64_t MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::GetWOffset(uint32_t tailN, uint32_t k)
{
    uint64_t wOffset = 0;
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::CubeProcess(MNConfig &mnConfig)
{
    if ASCEND_IS_AIC {
        preOffset = 0;
        int32_t prevSplitValue = 0;
        for (uint32_t loop = 0, count = 0; loop < gmmSwiglu->groupListLen * GROUP_LIST_INNER_SHAPE; loop+=GROUP_LIST_INNER_SHAPE) {
            // int64_t groupM = groupListGM.GetValue(groupIdx);
            // int32_t currSplitValue = static_cast<int32_t>(groupM);
            // int32_t splitValue = currSplitValue - prevSplitValue;
            // prevSplitValue = currSplitValue;
            int32_t splitValue = static_cast<int32_t>(groupListGM.GetValue(loop + 1));
            if (splitValue <= 0) {
                break;
            }
            uint32_t groupIdx = static_cast<int32_t>(groupListGM.GetValue(loop));
            mnConfig.mAxisBaseOffset += mnConfig.m;
            mnConfig.xBaseOffset += mnConfig.m * mnConfig.k;
            mnConfig.yBaseOffset += mnConfig.m * mnConfig.n;
            ...
            while (curBlock < curCount) {
                ...
                // printf("blockIdx %d groupIdx %d splitValue %d m %d k %d n %d blockDimM %d blockDimN %d mIdx %d nIdx %d\n",
                // blockIdx, groupIdx, splitValue, mnConfig.m, mnConfig.k, mnConfig.n, mnConfig.blockDimM, mnConfig.blockDimN, mnConfig.mIdx, mnConfig.nIdx);
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::VecProcess(VecConfig &vecConfig)
{
    if ASCEND_IS_AIV {
        ...
        if (blockIdx < vecConfig.usedCoreNum) {
            for (uint32_t outLoopIdx = 0; outLoopIdx < vecConfig.outLoopNum; outLoopIdx++) {
                ...
                for (uint32_t innerLoopIdx = 0; innerLoopIdx < vecConfig.innerLoopNum; innerLoopIdx++) {
                    // printf("blockIdx %d innerLoopNum %d innerLoopIdx %d nextupdateInterval %d\n",
                    // blockIdx, vecConfig.innerLoopNum, innerLoopIdx, vecConfig.nextUpadteInterVal);
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void
MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::MNBlockIdxCompute(MNConfig &mnConfig, const uint32_t curBlock,
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::UpdateVecConfig(uint32_t blockIdx,
                                                                                     VecConfig &vecConfig)
{
    // 第一步 读取grouplist reduceSum 计算总数据个数
    for (uint32_t loop = 0; loop < gmmSwiglu->groupListLen * GROUP_LIST_INNER_SHAPE; loop+=GROUP_LIST_INNER_SHAPE) {
        int64_t currM = groupListGM.GetValue(loop + 1);
        if (currM <= 0) {
            break;
        }
        vecConfig.M += currM;
    ...
    // printf("blockIdx %d curStartIdx %d\n", blockIdx, curStartIdx);
    for (uint32_t loop = 0; loop < gmmSwiglu->groupListLen * GROUP_LIST_INNER_SHAPE; loop+=GROUP_LIST_INNER_SHAPE) {
        int64_t currM = groupListGM.GetValue(loop + 1);
        int64_t groupIdx = groupListGM.GetValue(loop);
        if (currM <= 0) {
            break;
        }
        if (curStartIdx >= 0 && curStartIdx - currM < 0) {
            vecConfig.curGroupIdx = groupIdx;
            vecConfig.nextUpadteInterVal = currM - curStartIdx;
            vecConfig.groupRowIdx = loop / GROUP_LIST_INNER_SHAPE;

            // printf("blockIdx %d curGroupIdx %d curStartIdx %d nextUpadteInterVal %d groupRowIdx %d\n",
            // blockIdx, vecConfig.curGroupIdx, curStartIdx, vecConfig.nextUpadteInterVal, vecConfig.groupRowIdx);
        }
        curStartIdx -= currM;
    }
    // 第三步 计算总数据量
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::customDataCopyIn(uint32_t outLoopIdx,
                                                                                      VecConfig &vecConfig)
{
    ...
    // if (vecConfig.startIdx == 34) {
    //     printf("blockIdx %d groupIdx %d innerLoopNum %d\n", blockIdx, vecConfig.curGroupIdx, vecConfig.innerLoopNum);
    //     DumpTensor(_inMMLocal_1, 1, gmmSwiglu->tokenLen);
    // }
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
template <typename DTYPE_CS>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::UpdateChannelScale(uint32_t loopIdx,
                                                                                        VecConfig &vecConfig)
{
    // 更新perChannel
    if (unlikely(vecConfig.nextUpadteInterVal == 0)) {
        int64_t nextLoopIdx = vecConfig.groupRowIdx + 1;
        for (int64_t loop = nextLoopIdx; loop < gmmSwiglu->groupListLen; loop++) {
            int64_t splitValue = groupListGM.GetValue(loop * GROUP_LIST_INNER_SHAPE + 1);
            int64_t groupIdx = groupListGM.GetValue(loop * GROUP_LIST_INNER_SHAPE);
            if (splitValue > 0) {
                // printf("blockIdx %d groupIdx %d splitValue %d loop %d\n", blockIdx, groupIdx, splitValue, loop);
                vecConfig.nextUpadteInterVal = splitValue;
                vecConfig.curGroupIdx = groupIdx;
                vecConfig.groupRowIdx = loop;
                break;
            } 
            if (splitValue <= 0) {
                break;
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::VectorCompute(uint32_t loopIdx,
                                                                                   VecConfig &vecConfig)
{
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::Dequant(uint32_t loopIdx, VecConfig &vecConfig)
{
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::Swiglu(uint32_t loopIdx)
{
    // 高阶API swiglu
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::Quant(uint32_t loopIdx)
{
    LocalTensor<float> _inMMLocal = mmOutQueue.DeQue<float>();
...
template <typename mmType, bool sync, typename CHANNELDTYPE>
__aicore__ inline void MoeGMMSwigluCompute<mmType, sync, CHANNELDTYPE>::customDataCopyOut(VecConfig &vecConfig)
{
    // perChanelScale * perTokenScale
    LocalTensor<float> quantScaleLocal = quantScaleOutQueue.DeQue<float>();
