...
#include "moe_grouped_matmul_swiglu_quant.h"
#include <typeinfo>
#include "moe_grouped_matmul_swiglu_quant_split_ws.h"
using namespace AscendC;
using namespace matmul;
using namespace MOE_GROUPED_MATMUL_SWIGLU_QUANT;
...
#define GMM_CV_SPLIT_IMP(computeClass, dtypeWeightScale, transA, transB, sync)                                         \
    do {                                                                                                               \
        ...
        matmulType::MT mm;                                                                                             \
        GET_TILING_DATA_MEMBER(MoeGMMSwigluQuantTilingData, gmmSwigluBaseParams, gmmSwigluBaseParams_, tiling);           \
        GET_TILING_DATA_MEMBER(MoeGMMSwigluQuantTilingData, mmTilingData, mmTilingData_, tiling);                         \
        GET_TILING_DATA_MEMBER(MoeGMMSwigluQuantTilingData, gmmSwiglu, gmmSwiglu_, tiling);                               \
        if ASCEND_IS_AIC {                                                                                             \
            mm.SetSubBlockIdx(0);                                                                                      \
...
extern "C" __global__ __aicore__ void moe_grouped_matmul_swiglu_quant(GM_ADDR x, GM_ADDR weight, GM_ADDR weightScale,
                                                                  GM_ADDR xScale, GM_ADDR groupList, GM_ADDR y, 
                                                                  GM_ADDR yScale, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(0)) { // antiquant msd
        ...
        GMM_CV_SPLIT_IMP(MoeGMMSwigluCompute, // computeClass
                         DTYPE_WEIGHT_SCALE,
                         false, // transA
                         false, // transB
                         false  // sync
        ...
        GMM_CV_SPLIT_IMP(MoeGMMSwigluSplitWorkSpaceCompute, // computeClass
                         DTYPE_WEIGHT_SCALE,
                         false, // transA
                         false, // transB
                         false  // sync
