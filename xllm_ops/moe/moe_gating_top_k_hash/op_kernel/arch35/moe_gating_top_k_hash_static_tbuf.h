 ...
 * \brief
 */
#ifndef MOE_GATING_TOP_K_HASH_STATIC_TBUF_H
#define MOE_GATING_TOP_K_HASH_STATIC_TBUF_H

#include <cmath>
#include <type_traits>
#include "kernel_operator.h"
#include "kernel_utils.h"
#include "moe_gating_top_k_hash_static_common.h"

namespace MoeGatingTopKHashStaticTbufNS
{
    using namespace AscendC;
    using MicroAPI::RegTensor;

    // P=true is a one-row contract.  This adapter keeps the existing
    // producer/consumer call sites intact while backing every logical slot
    // with one TBuf allocation.  EnQue/DeQue are intentionally no-ops because
    // the static path has no queue ownership to transfer.
    class MoeGatingTopKHashSingleRowTBuf
    {
    public:
        __aicore__ inline void Init(TPipe *pipe, uint32_t bytes)
        {
            pipe->InitBuffer(buf_, bytes);
        }
        ...
        template <typename T>
        __aicore__ inline void EnQue(const LocalTensor<T> &)
        {
        }

        template <typename T>
        __aicore__ inline void FreeTensor(const LocalTensor<T> &)
        {
        }
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW = false,
              int32_t FAST_NORM = -1, bool FAST_HASH = false, bool FAST_BIAS = false,
              bool FAST_ALIGNED = false>
    class MoeGatingTopKHashStaticTbuf
    {
    public:
        __aicore__ inline MoeGatingTopKHashStaticTbuf(){};
    ...
    private:
        TPipe *pipe_;
        using XInQueue = std::conditional_t<STATIC_SINGLE_ROW, MoeGatingTopKHashSingleRowTBuf,
                                            TQue<QuePosition::VECIN, 1>>;
        using YOutQueue = std::conditional_t<STATIC_SINGLE_ROW, MoeGatingTopKHashSingleRowTBuf,
                                             TQue<QuePosition::VECOUT, 1>>;
        XInQueue xInQueue_;
        YOutQueue yOutQueue_;
        YOutQueue expertIdxOutQueue_;
        YOutQueue outOutQueue_;
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::CopyInBias()
    {
        if (!hasBias_)
        {
            return;
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::CopyInX(int64_t row)
    {
        DataCopyExtParams dataCopyParams;
        dataCopyParams.blockCount = groupCount_;
        dataCopyParams.blockLen = perGroupExpertCount_ * sizeof(T);
        ...
        {
            LocalTensor<float> xInLocalTensor = xInQueue_.template AllocTensor<float>();
        ...
        {
            LocalTensor<T> xInLocalTensor = xInQueue_.template AllocTensor<T>();
            DataCopyPad(xInLocalTensor, xGm_[row * expertCount_], dataCopyParams, dataCopyPadParams);
            xInQueue_.EnQue(xInLocalTensor);
        }
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::ComputeSoftmax()
    {
        ...
        LocalTensor<float> xInLocalTensor = xInQueue_.template DeQue<float>();
        if constexpr (!IsSameType<T, float>::value)
        {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::ComputeSigmoid()
    {
        ...
        uint32_t perGroupExpertCountAlign0 = perGroupExpertCountAlign_;
        uint16_t groupCount0 = groupCount_;
        LocalTensor<T> xInLocalTensor = xInQueue_.template DeQue<T>();
        if (hasBias_)
        {
            LocalTensor<T> biasTensor = biasBuf_.Get<T>();
            __VEC_SCOPE__
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::ComputeSoftplus()
    {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::ComputeX()
    {
        if constexpr (STATIC_SINGLE_ROW && FAST_NORM == 0) {
            ComputeSoftmax();
            return;
        } else if constexpr (STATIC_SINGLE_ROW && FAST_NORM == 1) {
            ComputeSigmoid();
            return;
        } else if constexpr (STATIC_SINGLE_ROW) {
            ComputeSoftplus();
            return;
        } else if (tilingData_->normType == 0) {
            ComputeSoftmax();
            return;
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::CopyOutXNorm(int64_t progress)
    {
        if (tilingData_->outFlag == 0)
        {
            return;
        }
        LocalTensor<float> outOutTensor = outOutQueue_.template AllocTensor<float>();
        LocalTensor<float> xSigmoidTensor = xSigmoidBuf_.Get<float>();
        DataCopy(outOutTensor, xSigmoidTensor, expertCountAlign_);
        if constexpr (STATIC_SINGLE_ROW) {
            SetWaitFlag<HardEvent::V_MTE3>(HardEvent::V_MTE3);
        }
        outOutQueue_.template EnQue<float>(outOutTensor);
        outOutTensor = outOutQueue_.template DeQue<float>();
        DataCopyExtParams dataCopyParams{
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::SortInGroup()
    {
        LocalTensor<float> xBiasTensor = xBiasBuf_.Get<float>();
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::TopKCompute()
    {
        LocalTensor<float> xBiasTensor = xBiasBuf_.Get<float>();
        LocalTensor<float> sortedInGroupTensor = sortedInGroupBuf_.Get<float>(); // 组内排序的结果, 后续归并需要
        LocalTensor<float> tmpLocal = finalSortBuffer_.Get<float>();
        ...
        LocalTensor<float> xSigmoidTensor = xSigmoidBuf_.Get<float>();
        LocalTensor<T> yTensor = yOutQueue_.template AllocTensor<T>();
        LocalTensor<int32_t> expertIdxTensor = expertIdxOutQueue_.template AllocTensor<int32_t>();
        ...
        expertIdxOutQueue_.template EnQue<int32_t>(expertIdxTensor);
    }

    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void
    MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::HashGatherWithSmallKAlignE(LocalTensor<float> xSigmoidTensor,
                                                        LocalTensor<int32_t> expertIdxTensor, LocalTensor<T> yTensor, uint32_t k,
                                                        float eps, float routedScalingFactor)
    {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void
    MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::HashGatherWithSmallKNotAlignE(LocalTensor<float> xSigmoidTensor,
                                                           LocalTensor<int32_t> expertIdxTensor, LocalTensor<T> yTensor, uint32_t k,
                                                           float eps, float routedScalingFactor, int32_t expertIdxPad,
                                                           int32_t perGroupExpertCountAlign)
    {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::HashCompute(int64_t row)
    {
        LocalTensor<int32_t> expertIdxTensor = expertIdxOutQueue_.template AllocTensor<int32_t>();
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::SelectTopKGroupIndex()
    {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::FinalSortByKGroup()
    {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::FinalSortAfterKGroup()
    {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void
    MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::TopKGatherWithSmallKAlignE(LocalTensor<float> xSigmoidTensor, LocalTensor<int32_t> mrgSortTensor,
                                                        LocalTensor<int32_t> expertIdxTensor, LocalTensor<T> yTensor, uint32_t k,
                                                        float eps, float routedScalingFactor)
    {
        __local_mem__ float *inputAddr = (__local_mem__ float *)xSigmoidTensor.GetPhyAddr();
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void
    MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::TopKGatherWithLargeKAlignE(LocalTensor<float> xSigmoidTensor, LocalTensor<int32_t> mrgSortTensor,
                                                        LocalTensor<int32_t> expertIdxTensor, LocalTensor<T> yTensor, uint32_t k,
                                                        float eps, float routedScalingFactor)
    {
        uint32_t k1 = k_;
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void
    MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::TopKGatherWithSmallKNotAlignE(LocalTensor<float> xSigmoidTensor, LocalTensor<int32_t> mrgSortTensor,
                                                           LocalTensor<int32_t> expertIdxTensor, LocalTensor<T> yTensor, uint32_t k,
                                                           float eps, float routedScalingFactor, int32_t expertIdxPad,
                                                           int32_t perGroupExpertCountAlign)
    {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void
    MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::TopKGatherWithLargeKNotAlignE(LocalTensor<float> xSigmoidTensor, LocalTensor<int32_t> mrgSortTensor,
                                                           LocalTensor<int32_t> expertIdxTensor, LocalTensor<T> yTensor, uint32_t k,
                                                           float eps, float routedScalingFactor, int32_t expertIdxPad,
                                                           int32_t perGroupExpertCountAlign)
    {
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::SelectTopKExpertScore()
    {
        LocalTensor<int32_t> expertIdxTensor = expertIdxOutQueue_.template AllocTensor<int32_t>();
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::CopyOut(int64_t row)
    {
        LocalTensor<T> yOutTensor = yOutQueue_.template DeQue<T>();
        LocalTensor<int32_t> expertIdxTensor = expertIdxOutQueue_.template DeQue<int32_t>();
        if constexpr (STATIC_SINGLE_ROW) {
            SetWaitFlag<HardEvent::V_MTE3>(HardEvent::V_MTE3);
        }
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::Init(GM_ADDR x, GM_ADDR bias, GM_ADDR inputIds, GM_ADDR tid2eid, GM_ADDR y, GM_ADDR expertIdx, GM_ADDR out,
                                                         GM_ADDR workspace,
                                                         const MoeGatingTopKHashRegbaseTilingData *tilingData, TPipe *tPipe)
    {
        ...
        // Under P the four queue slots become independent one-shot TBufs.
        // The old queue setup remains byte-for-byte available to fallback
        // instantiations.
        ...
        if constexpr (STATIC_SINGLE_ROW) {
            xInQueue_.Init(pipe_, expertGroupAlign * sizeof(float) * (sizeof(float) / sizeof(T)));
            yOutQueue_.Init(pipe_, AlignBytes(k_, sizeof(T)));
            expertIdxOutQueue_.Init(pipe_, AlignBytes(k_, sizeof(int32_t)));
            outOutQueue_.Init(pipe_, expertGroupAlign * sizeof(float));
        } else {
            pipe_->InitBuffer(xInQueue_, CONSTANT_TWO,
                              expertGroupAlign * sizeof(float) * (sizeof(float) / sizeof(T)));
    ...
    template <typename T, typename  U1, typename U2, bool STATIC_SINGLE_ROW,
              int32_t FAST_NORM, bool FAST_HASH, bool FAST_BIAS, bool FAST_ALIGNED>
    __aicore__ inline void MoeGatingTopKHashStaticTbuf<T, U1, U2, STATIC_SINGLE_ROW, FAST_NORM, FAST_HASH, FAST_BIAS, FAST_ALIGNED>::Process()
    {
        CopyInBias();
        if constexpr (STATIC_SINGLE_ROW) {
            // P fixes one row per core and group_count==k_group==1.  There is
            // no producer/consumer overlap to pipeline, so execute the row
            // lifecycle directly and avoid the multi-row state machine.
            CopyInX(0);
            ComputeX();
            CopyOutXNorm(0);
            if constexpr (FAST_HASH) {
                HashCompute(tilingData_->perCoreRowCount * blockIdx_);
            } else {
                TopKCompute();
            }
            CopyOut(0);
...
#endif // MOE_GATING_TOP_K_HASH_STATIC_TBUF_H
