...
graphStatus InferShape4DequantSwigluQuant(gert::InferShapeContext* context) {
  OPS_LOG_D(context, "Begin to do InferShape4DequantSwigluQuant.");
  ...
  *yShape = *xShape;
  OPS_CHECK(Ops::Base::IsUnknownRank(*xShape),
           OPS_LOG_D(context, "End to do InferShape4DequantSwigluQuant, inputx is [-2]."),
           return GRAPH_SUCCESS);

  auto attrsPtr = context->GetAttrs();
  OPS_LOG_E_IF_NULL(context, attrsPtr, return ge::GRAPH_FAILED);
  ...
  *scaleShape = *yShape;
  ...
  OPS_LOG_D(context, "End to do InferShape4DequantSwigluQuant");
  return ge::GRAPH_SUCCESS;
}

graphStatus InferDtype4DequantSwigluQuant(gert::InferDataTypeContext* context) {
  OPS_LOG_D(context, "InferDtype4DequantSwigluQuant enter");
  ...
  OPS_CHECK(std::find(Y_SUPPORT_DTYPE_SET.begin(), Y_SUPPORT_DTYPE_SET.end(), outDtype) == Y_SUPPORT_DTYPE_SET.end(),
  ...
  OPS_LOG_D(context, "InferDtype4DequantSwigluQuant end");
