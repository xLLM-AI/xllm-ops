...
std::tuple<at::Tensor, at::Tensor, at::Tensor> construct_sparse_flash_attention_lse_output_tensor(
    ...
    int64_t attention_mode, bool return_softmax_lse)
{
    ...
    auto sparse_flash_attention_lse_output =
        construct_sparse_flash_attention_lse_output_tensor(
            query, key, layout_query_str, layout_kv_str, return_softmax_lse);
    at::Tensor attention_output = std::get<0>(sparse_flash_attention_lse_output);
    at::Tensor softmax_max = std::get<1>(sparse_flash_attention_lse_output);
    at::Tensor softmax_sum = std::get<2>(sparse_flash_attention_lse_output);
