# aclnnSparseFlashAttentionLse

[📄 查看源码](https://gitcode.com/cann/ops-transformer/tree/master/attention/sparse_flash_attention_lse)
...
- 接口功能：sparse_flash_attention_lse（SFA）是针对大序列长度推理场景的高效注意力计算模块，该模块通过“只计算关键部分”大幅减少计算量，然而会引入大量的离散访存，造成数据搬运时间增加，进而影响整体性能。
...
每个算子分为[两段式接口](../../../docs/zh/context/两段式接口.md)，必须先调用“aclnnSparseFlashAttentionLseGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnSparseFlashAttentionLse”接口执行计算。

```Cpp
aclnnStatus aclnnSparseFlashAttentionLseGetWorkspaceSize(
...
```Cpp
aclnnStatus aclnnSparseFlashAttentionLse(
    void             *workspace, 
    uint64_t          workspaceSize, 
    aclOpExecutor    *executor, 
...
## aclnnSparseFlashAttentionLseGetWorkspaceSize
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnSparseFlashAttentionLseGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：aclnnSparseFlashAttentionLse默认确定性实现。
- 该接口支持推理场景下使用。
- N1支持1~64和128。
...
#include "securec.h"
#include "acl/acl.h"
#include "aclnnop/aclnn_sparse_flash_attention_lse.h"
...
int ExecuteSparseFlashAttentionLse(TensorResources& resources, aclrtStream stream, 
    ...
    int ret = aclnnSparseFlashAttentionLseGetWorkspaceSize(resources.queryTensor, resources.keyTensor, resources.valueTensor, resources.sparseIndicesTensor, nullptr, nullptr, nullptr, resources.queryRopeTensor, resources.keyRopeTensor,
    ...
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        LOG_PRINT("aclnnSparseFlashAttentionLseGetWorkspaceSize failed. ERROR: %d\n", ret);
        return ret;
    }
    ...
    ret = aclnnSparseFlashAttentionLse(*workspaceAddr, *workspaceSize, executor, stream);
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        LOG_PRINT("aclnnSparseFlashAttentionLse failed. ERROR: %d\n", ret);
        return ret;
    }
...
int main() {
    ...
    ret = ExecuteSparseFlashAttentionLse(resources, stream, &workspaceAddr, &workspaceSize);
    if (!CHECK_RET(ret == ACL_SUCCESS)) {
        CleanupResources(resources, workspaceAddr, stream, deviceId);
        return ret;
    }
