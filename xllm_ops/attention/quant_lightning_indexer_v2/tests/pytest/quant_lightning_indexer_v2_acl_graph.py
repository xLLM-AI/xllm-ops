...
def _qliv2_prepare_tensors_and_metadata(params, tensor_dict):
    ...
    cu_seqlens_query = (
        tensor_dict["cu_seqlens_query"].npu()
        if tensor_dict["cu_seqlens_query"] is not None
        else None
    )
    cu_seqlens_key = (
        tensor_dict["cu_seqlens_key"].npu()
        if tensor_dict["cu_seqlens_key"] is not None
        else None
    )
    seqused_q = (
        tensor_dict["seqused_q"].npu() if tensor_dict["seqused_q"] is not None else None
    )
    seqused_k = (
        tensor_dict["seqused_k"].npu() if tensor_dict["seqused_k"] is not None else None
    )
    output_idx_offset = (
        tensor_dict["output_idx_offset"].npu()
        if tensor_dict["output_idx_offset"] is not None
        else None
    )
    block_table = (
        tensor_dict["block_table"].npu()
        if tensor_dict["block_table"] is not None
        else None
    )
    cmp_residual_k_for_npu = (
        tensor_dict["cmp_residual_k_for_npu"].npu()
        if tensor_dict.get("cmp_residual_k_for_npu") is not None
        else None
    )
...
def _qliv2_run_compiled_graph(run_args):
    ...
    npu_mode = torch.compile(
        QLIV2Network().npu(), fullgraph=False, backend=npu_backend, dynamic=False
    )
