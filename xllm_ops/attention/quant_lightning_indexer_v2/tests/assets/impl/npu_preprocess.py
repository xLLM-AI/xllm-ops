...
def restore_mx_dtypes(query, key, query_scale, key_scale, quant_mode):
    ...
    for name, tensor, dtype in (
        ...
        if tensor.dtype != torch.uint8:
            raise TypeError(
                f"QLI_V2 {name} must use {dtype} or uint8 storage, got {tensor.dtype}"
            )
        tensor.data = tensor.data.view(dtype)


def build_metadata_arguments(
    query, key, topk, quant_mode, layout_q, layout_k, mask_mode, cmp_ratio, kwargs
):
...
def run(
    ...
    **kwargs,
    ...
    if tuple(metadata.shape) != tuple(generated.shape):
        raise ValueError(
            "QLI_V2 metadata shape mismatch: "
