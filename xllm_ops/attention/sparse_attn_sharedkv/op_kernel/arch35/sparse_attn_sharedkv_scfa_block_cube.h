...
private:
    ...
    static constexpr uint32_t M_SPLIT_SIZE = 128;     // m-direction split
    static constexpr uint32_t N_SPLIT_SIZE = 128;     // n-direction split
    static constexpr uint32_t K_L0_SPLIT_SIZE = 128;  // k-direction L0 split
    static constexpr uint32_t K_L1_SPLIT_SIZE = 256;  // k-direction L1 split
    static constexpr uint32_t N_WORKSPACE_SIZE = 512; // n-direction split
    static constexpr uint32_t D_SPLIT_SIZE = 256; // d-axis split
    ...
    static constexpr uint32_t mte21QPIds[4] = {L1_EVENT0, L1_EVENT1, L1_EVENT2, L1_EVENT3}; // mte12 reuse
    static constexpr uint32_t mte21KVIds[3] = {L1_EVENT4, L1_EVENT5, L1_EVENT6};

    ConstInfo constInfo{};

    // L1 is divided into 3 buf blocks, used for recording
    ...
    {
        uint32_t idxMap[] = {0, 2}; // Ensure block 0 and block 1 are adjacent, block 2 and block 3 are adjacent, to keep addresses of the same m block contiguous
                                                      ...
                                                      uint32_t srcN, uint32_t srcD, uint32_t srcDstride)
{
    ...
    nd2nzPara.dstNzC0Stride = (srcN + 15) / 16 * 16; // align to 16-unit block
...
__aicore__ inline void SASCubeBlock<SAST>::ComputeMm1(const RunInfo &info, const MSplitInfo mSplitInfo)
{
    ...
    for (uint32_t nL1 = 0; nL1 < nL1Loops; nL1++) {
        if (nL1 == (nL1Loops - 1)) {
            // recompute size for the tail block
        ...
        for (uint32_t kL1 = 0; kL1 < kL1Loops; kL1++) {
            ...
            if (info.isOri) {
                if constexpr (KV_LAYOUT_T == SAS_LAYOUT::PA_ND) {
                    ...
                    while (copyFinishRowCnt < nL1Size) {
                        // Due to the existence of ori_left, even the first moved block may not be the zero position of pa_block
                        ...
                        startPos.dIdx = kL1 * D_SPLIT_SIZE;  // mm1 right matrix bn2s2d, d is the k axis and is not split; mm2 right matrix, s2 is the k axis, d axis is split
            ...
            for (uint32_t mL1 = 0; mL1 < mL1Loops; mL1++) {
                uint32_t aL1PaddingSize = 0; // used to align the left matrix to the tail so that the two 32K memory blocks are contiguous
                if (mL1 == (mL1Loops - 1)) {
                ...
                // synchronize using unitflag
                LocalTensor cL0Tensor =
                    cL0TensorPingPong[(cL0BufIter % 2) *
                                      (L0C_PP_SIZE / sizeof(MM_OUT_T))]; // must keep cL0BufIter consistent with the m pace
                for (uint32_t kL0 = 0; kL0 < kL0Loops; kL0++) {
                    ...
                    mmadParams.unitFlag =
                        (kL1 == 1 && kL0 == (kL0Loops - 1)) ? 0b11 : 0b10; // flip the flag on the last accumulation, indicating it can be moved out
                ...
                if (nL1 == (nL1Loops - 1)) {
                    SetFlag<HardEvent::MTE1_MTE2>(mte21QPIds[ka]); // reverse synchronization, indicating A in L1 has been fully consumed by mte1
                }

                if (kL1 == 1) { // last round of the kL1 loop
                    FixpipeParamsV220 fixParams;
                    fixParams.nSize = nL1SizeAlign;
                    fixParams.mSize = mL1SizeAlign;
                    fixParams.srcStride = mL1SizeAlign;
                    // change to nSizeAlign
                    fixParams.dstStride = info.actualSingleProcessSInnerSizeAlign; // interval between two rows of mm1ResGm
                    fixParams.unitFlag = 0b11;
                    fixParams.ndNum = 1; // output ND
            ...
            SetFlag<HardEvent::MTE1_MTE2>(mte21KVIds[kb]); // reverse synchronization, indicating L1 has been fully consumed by mte1
        }
        if (mL1Loops == 1) {
            cL0BufIter++;
        }
...
__aicore__ inline void SASCubeBlock<SAST>::ComputeMm2(const RunInfo &info, const MSplitInfo mSplitInfo)
{
    ...
    uint32_t mL1SizeAlign = M_SPLIT_SIZE; // 16-aligned
    uint32_t mL1Size = M_SPLIT_SIZE;      // actual size of m

    uint32_t nSize = BlockAlign<KV_T>(constInfo.headDim);
    uint32_t nL1Loops = (nSize + N_SPLIT_SIZE - 1) / N_SPLIT_SIZE;
    uint32_t nL1SizeAlign = N_SPLIT_SIZE; // 16-aligned
    uint32_t nL1Size = N_SPLIT_SIZE;      // actual size of n
    ...
    // ka indicates which buf of the left matrix's 4 bufs is selected, kb indicates which buf of the right matrix's 3 bufs is selected
    uint32_t ka = 0, kb = 0;
    uint32_t mBaseIdx = qpL1BufIter;
    for (uint32_t nL1 = 0; nL1 < nL1Loops; nL1++) { // n split L1 -> D
        if (nL1 == (nL1Loops - 1)) {
            // tail block
            nL1Size = nSize - (nL1Loops - 1) * N_SPLIT_SIZE;
            nL1SizeAlign = SASAlign(nL1Size, 16U);
        }
        // write k l1 as a single loop, consistent with mm1
        kL1Size = 256;
        kL1SizeAlign = SASAlign(kL1Size, 16U);
        uint32_t copyRowCnt = 0;

        for (uint32_t k1 = 0; k1 < kL1Loops; k1++) { // k split L1, an l0 layer is nested here to operate -> S2, 256 each time
            if (k1 == (kL1Loops - 1)) {
                // tail block
            ...
            kL0Size = 128;
            // kL0Size must be initialized first, then kL0Loops computed, otherwise the loop would change kL0Size and cause an incorrect kL0Loops
            kL0Loops = (kL1Size + kL0Size - 1) / kL0Size;
            kL0SizeAlign = kL0Size;
            for (uint32_t kL1 = kOffset; kL1 < kL0Loops + kOffset; kL1++) { // 128 loop moving pa, 128 each time
                ...
                if (info.isOri) {
                    if constexpr (KV_LAYOUT_T == SAS_LAYOUT::PA_ND) {
                        ...
                        while (copyFinishRowCnt < kL0Size) {
                            ...
                            startPos.n2Idx = info.n2Idx;
                            startPos.s2Idx = curS2Offset;
                            startPos.dIdx = nL1 * N_SPLIT_SIZE;  // mm1 right matrix bn2s2d, d is the k axis and is not split; mm2 right matrix, s2 is the k axis, d axis is split
                            PAShape shape;
                            shape.blockSize = constInfo.paOriBlockSize;
                            shape.headNum = constInfo.kvHeadNum;
                            shape.headDim = constInfo.headDim;
            ...
            mL1Size = M_SPLIT_SIZE; // actual size of m
            for (uint32_t mL1 = 0; mL1 < mL1Loops; mL1++) {
                if (mL1 == (mL1Loops - 1)) {
                    // tail block
                    mL1Size = mSize - (mL1Loops - 1) * M_SPLIT_SIZE;
                ...
                for (uint32_t kL0 = 0; kL0 < kL0Loops; kL0++) {
                    ...
                    loadData3DParamsForB.l1W = 16;                // source operand weight=16, destination height=l1H*L1W
                    ...
                    loadData3DParamsForB.mExtension = kL0SizeAlign; // transfer length in the destination operand's height dimension
                    loadData3DParamsForB.kExtension = nL1SizeAlign; // transfer length in the destination operand's width dimension
                    loadData3DParamsForB.mStartPt = 0;              // start point of the convolution kernel in the destination operand's width dimension
                    loadData3DParamsForB.kStartPt = 0;              // start point of the convolution kernel in the destination operand's height dimension
                    loadData3DParamsForB.strideW = 1;
                    loadData3DParamsForB.strideH = 1;
                    loadData3DParamsForB.filterW = 1;
                    loadData3DParamsForB.filterSizeW = false; // whether to increase the convolution kernel width by 256 elements on top of filterW
                    loadData3DParamsForB.filterH = 1;
                    loadData3DParamsForB.filterSizeH = false; // whether to increase the convolution kernel height by 256 elements on top of filterH
                    loadData3DParamsForB.dilationFilterW = 1; // convolution kernel width dilation factor
                    loadData3DParamsForB.dilationFilterH = 1; // convolution kernel height dilation factor
                    loadData3DParamsForB.enTranspose = 1;     // whether to enable the transpose function
                    loadData3DParamsForB.fMatrixCtrl =
                        0; // use FMATRIX_LEFT or FMATRIX_RIGHT, =0 uses FMATRIX_LEFT, =1 uses FMATRIX_RIGHT 1
                    loadData3DParamsForB.channelSize =
                        nL1SizeAlign; // number of channels of the source operand. When the dilation factor is 1, the destination weight is filterW*filterH*channelSize
                    ...
                    loadData3DParamsForA.l1W = 16;                // source operand weight
                    ...
                    loadData3DParamsForA.mExtension = mL1SizeAlign; // transfer length in the destination operand's height dimension
                    loadData3DParamsForA.kExtension = kL0SizeAlign; // transfer length in the destination operand's width dimension
                    loadData3DParamsForA.mStartPt = 0;              // start point of the convolution kernel in the destination operand's width dimension
                    loadData3DParamsForA.kStartPt = 0;              // start point of the convolution kernel in the destination operand's height dimension
                    loadData3DParamsForA.strideW = 1;         // stride of the convolution kernel sliding in the source operand's width dimension
                    loadData3DParamsForA.strideH = 1;         // stride of the convolution kernel sliding in the source operand's height dimension
                    loadData3DParamsForA.filterW = 1;         // convolution kernel width
                    loadData3DParamsForA.filterSizeW = false; // whether to increase the convolution kernel width by 256 elements on top of filterW
                    loadData3DParamsForA.filterH = 1;         // convolution kernel height
                    loadData3DParamsForA.filterSizeH = false; // whether to increase the convolution kernel height by 256 elements on top of filterH
                    loadData3DParamsForA.dilationFilterW = 1; // convolution kernel width dilation factor
                    loadData3DParamsForA.dilationFilterH = 1; // convolution kernel height dilation factor
                    loadData3DParamsForA.enTranspose = 0; // whether to enable transpose, transposing the entire target matrix
                    loadData3DParamsForA.fMatrixCtrl = 0;
                    loadData3DParamsForA.channelSize =
                        kL0SizeAlign; // number of channels of the source operand. When the dilation factor is 1, the destination weight is filterW*filterH*channelSize
                ...
                if (nL1 == (nL1Loops - 1)) { // last round of nL1, B needs to stay resident in L1 for the next round of computation?
                    SetFlag<HardEvent::MTE1_MTE2>(mte21QPIds[ka]); // reverse synchronization, indicating A in L1 has been fully consumed by mte1
                }
                ...
                if (k1 == (kL1Loops - 1)) {
                    ...
                    fixParams.mSize = mL1SizeAlign;
                    fixParams.srcStride = mL1SizeAlign;
                    fixParams.dstStride = nSize; // interval between two rows of mm2ResGm
                    fixParams.ndNum = 1;         // output ND
                    fixParams.unitFlag = 0b11;
