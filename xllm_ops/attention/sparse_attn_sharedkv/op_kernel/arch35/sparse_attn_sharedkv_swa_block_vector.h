...
public:
    ...
    // FP32 element count of BLOCK and REPEAT
...
__aicore__ inline void SWAVectorBlock<SAST>::InitBuffers(TPipe *pipe)
{
    ...
    // Allocate 256+N1 size memory, where 256 is the maximum VEC tile of the m axis
...
__aicore__ inline void SWAVectorBlock<SAST>::CopySinksIn()
{
    ...
    repeatParams.blockCount = 1; // Move until a block exceeds the M-axis size of a single vec core after subdivision; inter-core split gives each vec 256
...
{
    // Since the repeatTimes of the WholeReduceMax interface supports range (0,255), WholeReduceMax must be called multiple times; here repeatTime=128 is used each time
...
__aicore__ inline void SWAVectorBlock<SAST>::ProcessVec1SingleBuf(const RunInfo &info, const MSplitInfo &mSplitInfo)
{
    ...
    // 1. Align down to 8 because UB operation is at least 32B
                                                               ...
                                                               uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    uint64_t offset =
        ...
        info.tndCoreStartKVSplitPos * constInfo.kvHeadNum * constInfo.mBaseSize * constInfo.headDim + // share count offset
                                                                  ...
                                                                  uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    // Output the computation result on the last iteration, otherwise store the intermediate result to workspace
                                                     ...
                                                     uint32_t actualColumnCount)
{
    ...
    if constexpr (std::is_same<T, half>::value) {
        // This limitation is because each repeat reads at most 256B of contiguous data
        repeatElementNum = FP32_REPEAT_ELEMENT_NUM * 2; // 256/4 * 2=128
        blockElementNum = FP32_BLOCK_ELEMENT_NUM * 2;   // 32/4 * 2 = 16
    }

    // Only 256B of contiguous data can be read for computation each time, so only 256B/sizeof(dType)= can be processed each time
    // Split the column direction into dLoop iterations, processing 8 columns of data each time
    uint32_t dLoop = actualColumnCount / repeatElementNum;
    uint32_t dRemain = actualColumnCount % repeatElementNum;
    // REPEATE_STRIDE_UP_BOUND=256, this limitation is because the src0RepStride data type is uint8 with at most 256 datablock intervals
    if (columnCount < REPEATE_STRIDE_UP_BOUND * blockElementNum) {
        ...
        // If the repeat count processed by column is less than that by row, process by column; otherwise process the repeat by row
        ...
        // The last time completes [dealRowCount, dRemain] * [dealRowCount, blockElementNum], computing only the valid part
        if (dRemain > 0) {
        ...
        for (uint32_t i = 0; i < dealRowCount; i++) {
            // Compute dLoop repeats in one row, each repeat computes 256/block_size data_blocks
