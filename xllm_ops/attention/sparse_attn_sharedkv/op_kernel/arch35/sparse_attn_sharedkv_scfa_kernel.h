...
// Before the S2 loop, RunInfo is not yet assigned, so Bngs1Param temporarily holds info about the B, N and S1 axes; this also reduces redundant computation
struct TempLoopInfo {
    uint32_t bn2IdxInCurCore = 0;
    uint32_t bIdx = 0U;
    uint32_t n2Idx = 0U;
    uint64_t s2BasicSizeTail = 0U; // Tail basic block size of the S2-direction loop
    uint32_t s2LoopTimes = 0U; // Total number of S2-direction loops; equals the actual count for both TND and BXXD, no need to subtract 1

    int32_t actS1Size = 0; // In the TND scenario, the S1-axis size processed by the current batch loop
    ...
    uint32_t s1EndIdx = 0;
    uint64_t mBasicSizeTail = 0U; // Tail basic block size of the gS1-direction loop
...
__aicore__ inline void SparseAttnSharedkvScfa<SAST>::InitTilingData()
{
    ...
    constInfo.sparseBlockSize = 1; // sparseBlockSize is fixed to 1
...
__aicore__ inline void SparseAttnSharedkvScfa<SAST>::InitAllZeroOutput(uint32_t bIdx, uint32_t s1Idx, uint32_t n2Idx)
{
    if (constInfo.outputLayout == SAS_LAYOUT::TND) {
        ...
        uint64_t attenOutOffset = (tBase + s1Idx) * kvHeadNum * constInfo.gSize * headDim + // T-axis and s1-axis offset
...
{
    // Row invalidity is judged by the ori part; if the ori part has invalid rows, both ori and cmp have them
    ...
    TPipe *tPipe)
{
    ...
    // Must be executed after InitParams
    if (pipe != nullptr) {
        InitBuffers();
    }
}
                                                                ...
                                                                uint32_t s2LoopIdx, RunInfo &info)
{
    ...
    if (info.isFirstSInnerLoop) {
        ...
        tensorBCoreOffset = tndBIdxOffsetForKV + info.n2Idx * constInfo.headDim; // Currently in the PA scenario, this variable is invalid
    ...
    if (s2LoopIdx < tempLoopInfo.oriLoopTimes) {
        // The first S2 loop can only be in ori_kv
        info.isOri = true;
...
__aicore__ inline void SparseAttnSharedkvScfa<SAST>::ProcessBalance()
{
    ...
    // Adapt to left-closed right-open interval
    ...
    for (uint32_t bN2LoopIdx = constInfo.bN2Start; bN2LoopIdx < constInfo.bN2End; bN2LoopIdx++) {
        ...
        // When at the last BN2 and gS1End is 0, all data in the current BN2 is processed by the current core
        gS1LoopEnd = (bN2LoopIdx + 1 == constInfo.bN2End && constInfo.gS1End != 0) ? constInfo.gS1End : gS1SplitNum;
        // When at the last BN2 and current S1 is 0, enter the loop to compute the unfinished part caused by preload
        ...
        for (uint32_t gS1LoopIdx = constInfo.gS1Start; gS1LoopIdx < gS1LoopEnd; gS1LoopIdx++) {
            ...
            // Compute required data to avoid redundant computation
            ...
            // These are all closed intervals
            ...
            // Needs to be enabled after core partitioning modification
            // Whether the current s2 is split determines whether the output must be written to attenOut
            ...
            for (uint32_t s2LoopIdx = constInfo.s2Start; s2LoopIdx < (s2LoopEnd + extraLoop); s2LoopIdx++) {
                ...
                ++gloop;
                if (s2LoopIdx >= tempLoopInfo.oriLoopTimes && s2LoopIdx < s2LoopEnd) { // Used to determine the id of the loop GM used by v0
                    ++cmpLoop;
                }
            }
...
__aicore__ inline void SparseAttnSharedkvScfa<SAST>::PreloadPipeline(uint32_t loop, uint32_t cmpLoop, uint64_t s2Start,
                                                                     ...
                                                                     RunInfo extraInfo[SAS_PRELOAD_TASK_CACHE_SIZE])
{
    RunInfo &extraInfo0 = extraInfo[loop % SAS_PRELOAD_TASK_CACHE_SIZE];       // Current-round task
    RunInfo &extraInfo2 = extraInfo[(loop + 2) % SAS_PRELOAD_TASK_CACHE_SIZE]; // Previous-round task
    RunInfo &extraInfo1 = extraInfo[(loop + 1) % SAS_PRELOAD_TASK_CACHE_SIZE]; // Task from two rounds ago
