...
__aicore__ inline void SASVectorBlock<SAST>::InitBuffers(TPipe *pipe)
{
    ...
    // allocate memory of size 256+N1, where 256 is the maximum VEC block size on the m axis
...
__aicore__ inline void SASVectorBlock<SAST>::CopySinksIn()
{
    ...
    uint32_t repeatTimes = (constInfo.qHeadNum + BLOCK_ELEMENT_NUM - 1U) / BLOCK_ELEMENT_NUM; // process 8 datablocks each time
    Brcb(sinksBrcbUb, sinksUb, repeatTimes, {1, BLOCK_ELEMENT_NUM});
    PipeBarrier<PIPE_V>();

    DataCopyParams repeatParams;
    repeatParams.blockCount = 1; // move until one block exceeds the per-vec-core M-axis size; inter-core split is 256 per vec
...
{
    // The repeatTimes of WholeReduceMax supports the range (0,255), so WholeReduceMax must be called multiple times, with repeatTime=128 each time
                                                                  ...
                                                                  uint32_t columnCount, uint32_t loopId)
{
    ...
    pingpongFlag ^= 1; // pingpong 0/1 switch
...
__aicore__ inline void SASVectorBlock<SAST>::ProcessVec1SingleBuf(const RunInfo &info, const MSplitInfo &mSplitInfo)
{
    ...
    // 1. rounding down to 8 alignment because UB operations are at least 32B
    // 2. info.actualSingleProcessSInnerSizeAlign is at most 512, mSplitSize can ensure a minimum of 16
                                                      ...
                                                      int64_t realS2Idx1, int64_t realS2Idx2, const RunInfo &runInfo)
{
    ...
    if (unlikely(keySrcStride >= INT32_MAX || keySrcStride < 0 ||
        ...
        // for abnormal cases such as stride overflow, negative stride, or over-long s2, fall back to 2 move instructions
        // because two blocks need to be copied
                                                               ...
                                                               uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    uint64_t offset =
        ...
        info.tndCoreStartKVSplitPos * constInfo.kvHeadNum * constInfo.mBaseSize * constInfo.headDim + // count offset
                                                                ...
                                                                uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    WaitFlag<AscendC::HardEvent::MTE3_V>(SYNC_OUTPUT_BUF1_FLAG);
    if constexpr (IsSameType<OUT_T, bfloat16_t>::value) { // bf16 uses round-half-to-even mode
                                                                  ...
                                                                  uint32_t columnCount, uint32_t actualColumnCount)
{
    ...
    // except for the first loop, the intermediate computation result must be updated
    if (!info.isFirstSInnerLoop) {
        ...
        LocalTensor<T> expUb = v0ValidSizeBuff.Get<T>()[384]; // sumUb uses temporary memory 16 * 32B = 512B
    ...
    // output the result on the last iteration; otherwise store the intermediate result in workspace
    if (info.isLastS2Loop) {
        uint32_t idx = info.loop % (constInfo.preLoadNum);
        LocalTensor<T> tmpSumUb = v0ValidSizeBuff.Get<T>()[384]; // sumUb uses temporary memory 16 * 32B = 512B
...
__aicore__ inline void SASVectorBlock<SAST>::RowDivs(LocalTensor<float> dstUb, LocalTensor<float> src0Ub,
                                                     ...
                                                     uint32_t columnCount, uint32_t actualColumnCount)
{
    // divs by row, elements of each row are divided by the same element
    ...
    if (columnRepeatCount <= dealRowCount) {
        ...
        columnRepeatParams.src0RepStride = 8; // in the column direction, the start-address interval between two repeats is dtypeMask=64 elements, i.e. 8 blocks
        columnRepeatParams.src1RepStride = 0;
        columnRepeatParams.dstRepStride = 8; // in the column direction, the start-address interval between two repeats is dtypeMask=64 elements, i.e. 8 blocks
        uint32_t offset = 0;
...
__aicore__ inline void SASVectorBlock<SAST>::RowMuls(LocalTensor<T> dstUb, LocalTensor<T> src0Ub, LocalTensor<T> src1Ub,
                                                     ...
                                                     uint32_t actualColumnCount)
{
    // muls by row, elements of each row are multiplied by the same element
    // dstUb[i, (j * 8) : (j * 8 + 7)] = src0Ub[i, (j * 8) : (j * 8 + 7)] * src1Ub[i, 0 : 7]
    // src0Ub:[dealRowCount, columnCount] src1Ub:[dealRowCount, FP32_BLOCK_ELEMENT_NUM] dstUb:[dealRowCount,
    // columnCount]
    ...
    if constexpr (std::is_same<T, half>::value) {
        // this limit is because each repeat can read at most 256B of contiguous data
        repeatElementNum = FP32_REPEAT_ELEMENT_NUM * 2; // 256/4 * 2=128
        blockElementNum = FP32_BLOCK_ELEMENT_NUM * 2;   // 32/4 * 2 = 16
    }

    // each time only 256B of contiguous data can be read for computation, so only 256B/sizeof(dType)= can be processed each time
    // the column direction is split into dLoop iterations, processing 8 columns of data each time
    uint32_t dLoop = actualColumnCount / repeatElementNum;
    uint32_t dRemain = actualColumnCount % repeatElementNum;
    // REPEATE_STRIDE_UP_BOUND=256; this limit is because src0RepStride is of type uint8, allowing at most a 256-datablock interval
    if (columnCount < REPEATE_STRIDE_UP_BOUND * blockElementNum) {
        ...
        // if the number of repeats processed by column is fewer than by row, process by column; otherwise process by row
        ...
        // the last iteration completes [dealRowCount, dRemain] * [dealRowCount, blockElementNum], computing only the valid part
        if (dRemain > 0) {
        ...
        BinaryRepeatParams repeatParams;
        repeatParams.src0RepStride = 8; // each repeat is 256B of data, exactly 8 datablocks
        repeatParams.src0BlkStride = 1;
        repeatParams.src1RepStride = 0;
        repeatParams.src1BlkStride = 0;
        repeatParams.dstRepStride = 8;
        repeatParams.dstBlkStride = 1;
        // compute one row each time, computing dealRowCount rows in total
        for (uint32_t i = 0; i < dealRowCount; i++) {
            // compute dLoop repeats within one row, each repeat computes 256/block_size data_blocks
            Mul(dstUb[i * columnCount], src0Ub[i * columnCount], src1Ub[i * blockElementNum], repeatElementNum, dLoop,
                repeatParams);
            //  compute the tail block within one row
            if (dRemain > 0) {
