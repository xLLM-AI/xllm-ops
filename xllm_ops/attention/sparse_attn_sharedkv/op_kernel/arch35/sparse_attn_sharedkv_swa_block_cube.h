                                                        ...
                                                        uint32_t kSize)
{
    ...
    // arch35: 3D convolution loading degenerates to 2D matrix loading (refer to compressor LoadAToL0)
    LoadData2DParamsV2 loadData2DParamsV2;
    loadData2DParamsV2.mStartPosition = 0;
    loadData2DParamsV2.kStartPosition = 0;
    loadData2DParamsV2.mStep = (mSize + 15) / 16;
    loadData2DParamsV2.kStep = (kSize + 15) / 16;
 ...
 LoadData(aL0Tensor, srcTensor, loadData2DParamsV2);
}
...
__aicore__ inline void SWACubeBlock<SAST>::ComputeMm1(const RunInfo &info, const MSplitInfo mSplitInfo)
{
    ...
    // L1 split n and k
    for (uint32_t nL1 = 0; nL1 < nL1Loops; nL1++) { // L1 split n, 512/128=4
        if (nL1 == (nL1Loops - 1)) {
        ...
        for (uint32_t kL1 = 0; kL1 < kL1Loops; kL1++) {
            ...
            // FIX: clear L1 KV padding rows [nL1Size, nL1SizeAlign) to avoid dirty buffer reuse polluting QK^T tail columns
            if (nL1SizeAlign > nL1Size) {
                AscendC::InitConstValue<KV_T>(bL1Tensor,
                    AscendC::InitConstValueParams<KV_T>(1, (uint16_t)(nL1SizeAlign * 256 / 32), 0, (KV_T)0));
            ...
            if (info.isOri) {
                if constexpr (KV_LAYOUT_T == SAS_LAYOUT::PA_ND) {
                    ...
                    while (copyFinishRowCnt < nL1Size) {
                        ...
                        // 256, 32 pending 7buf naming change
                        startPos.dIdx = kL1 * 256;  // mm1 right matrix bn2s2d, d is the k axis and is not split; mm2 right matrix, s2 is the k axis, d axis is split
                        PAShape shape;
...
__aicore__ inline void SWACubeBlock<SAST>::ComputeMm2(const RunInfo &info, const MSplitInfo mSplitInfo)
{
    ...
    for (uint32_t nL1 = 0; nL1 < nL1Loops; nL1++) { // n split L1
        if (nL1 == (nL1Loops - 1)) {
            // tail block
            nL1Size = nSize - (nL1Loops - 1) * N_SPLIT_SIZE;
        ...
        uint32_t copyRowCnt = 0;
        for (uint32_t k1 = 0; k1 < kL1Loops; k1++) { // k split L1, an l0 layer is nested here to operate
            if (k1 == (kL1Loops - 1)) {
                // tail block
            ...
            for (uint32_t kL1 = kOffset; kL1 < kL0Loops + kOffset; kL1++) { // 128 loop moving pa
            ...
            for (uint32_t mL1 = 0; mL1 < mL1Loops; mL1++) {
                ...
                for (uint32_t kL0 = 0; kL0 < kL0Loops; kL0++) {
                    ...
                    WaitFlag<HardEvent::M_MTE1>(Mte1MmABEventId(abL0BufIter % 2));
                    LocalTensor<KV_T> bL0Tensor = bL0TensorPingPong[(abL0BufIter % 2) * (L0B_PP_SIZE / sizeof(KV_T))];
                    // arch35: 3D loading degenerates to 2D matrix loading (transpose B, enTranspose=1 -> ifTranspose=true)
                    LoadData2DParamsV2 loadData2DParamsForB;
                    loadData2DParamsForB.mStartPosition = 0;
                    loadData2DParamsForB.kStartPosition = 0;
                    loadData2DParamsForB.mStep = (kL0SizeAlign + 15) / 16;
                    loadData2DParamsForB.kStep = (nL1SizeAlign + 15) / 16;
                    loadData2DParamsForB.srcStride = (kL0SizeAlign + 15) / 16;
                    loadData2DParamsForB.dstStride = loadData2DParamsForB.mStep;
                    loadData2DParamsForB.ifTranspose = true;
                    LoadData(bL0Tensor, bL1Tensor[kL0 * baseK * baseN], loadData2DParamsForB);

                    LocalTensor<KV_T> aL0Tensor = aL0TensorPingPong[(abL0BufIter % 2) * (L0A_PP_SIZE / sizeof(KV_T))];
                    // arch35: 3D loading degenerates to 2D matrix loading (no transpose, refer to compressor LoadAToL0)
                    LoadData2DParamsV2 loadData2DParamsForA;
                    loadData2DParamsForA.mStartPosition = 0;
                    loadData2DParamsForA.kStartPosition = 0;
                    loadData2DParamsForA.mStep = (mL1SizeAlign + 15) / 16;
                    loadData2DParamsForA.kStep = (kL0SizeAlign + 15) / 16;
                    loadData2DParamsForA.srcStride = (mL1SizeAlign + 15) / 16;
                    loadData2DParamsForA.dstStride = loadData2DParamsForA.mStep;
                    loadData2DParamsForA.ifTranspose = false;
                    LoadData(aL0Tensor, aL1Tensor[kL0 * baseK * mL1SizeAlign], loadData2DParamsForA);
                    SetFlag<HardEvent::MTE1_M>(Mte1MmABEventId(abL0BufIter % 2));
                    WaitFlag<HardEvent::MTE1_M>(Mte1MmABEventId(abL0BufIter % 2));
