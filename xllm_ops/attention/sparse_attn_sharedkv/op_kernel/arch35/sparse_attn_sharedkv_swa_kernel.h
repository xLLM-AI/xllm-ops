...
// Before the S2 loop, RunInfo is not yet assigned, so use Bngs1Param to temporarily store info related to the B, N, S1 axes; this also reduces redundant computation
struct SwaTempLoopInfo {
    uint32_t bn2IdxInCurCore = 0;
    uint32_t bIdx = 0U;
    uint32_t n2Idx = 0U;
    uint64_t s2BasicSizeTail = 0U; // Tail basic block size of the S2 direction loop
    uint32_t s2LoopTimes = 0U; // Total number of S2 direction loop iterations; equals the actual count for both TND and BXXD, no need to subtract 1

    int32_t actS1Size = 0; // Size of the S1 axis processed in the current Batch loop under the TND scenario
...
{
    // Row invalidity is determined by the ori part; if the ori part has invalid rows, then both ori and cmp have them
    ...
    // For the cmp part there is also top k; tempLoopInfo.actS2Size applies only to cmp
    if (constInfo.templateMode == CFA_TEMPLATE) {
...
__aicore__ inline void SparseAttnSharedkvSwa<SAST>::ProcessBalance()
{
    ...
    for (uint32_t bN2LoopIdx = constInfo.bN2Start; bN2LoopIdx < constInfo.bN2End; bN2LoopIdx++) {
        ...
        // When at the last BN2 and gS1End is 0, it means all data in the current BN2 is processed by the current core
        gS1LoopEnd = (bN2LoopIdx == constInfo.bN2End - 1 && constInfo.gS1End != 0) ? constInfo.gS1End : gS1SplitNum;
        // When at the last BN2 and the current S1 is 0, we need to enter the loop to compute the unfinished part caused by preload
        ...
        for (uint32_t gS1LoopIdx = constInfo.gS1Start; gS1LoopIdx < gS1LoopEnd; gS1LoopIdx++) {
            ...
            // For the real idx on each axis, use a left-closed right-closed scheme
            ...
            // Both are closed intervals here
            ...
            // Needs to be enabled after core splitting modification
            // Whether the current s2 is split determines whether the output should be written to attenOut
            ...
            for (uint32_t s2LoopIdx = constInfo.s2Start; s2LoopIdx < (s2LoopEnd + extraLoop); s2LoopIdx++) {
                // PreloadPipeline loop requires an initial value of PRELOAD_NUM
                                                                    ...
                                                                    RunInfo extraInfo[SAS_PRELOAD_TASK_CACHE_SIZE])
{
    ...
    RunInfo &extraInfo1 = extraInfo[(loop + 1) % SAS_PRELOAD_TASK_CACHE_SIZE]; // Two rounds ago task
