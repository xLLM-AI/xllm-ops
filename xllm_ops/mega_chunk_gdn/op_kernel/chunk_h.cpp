...
AICORE void chunk_h_kernel(
    __gm__ ComputeT *K_handle, __gm__ ComputeT *W_handle, __gm__ ComputeT *U_handle,
    ...
    uint64_t ffts_addr)
{
  ...
  for (int64_t wi = 0; wi < (total_work + block_num - 1) / block_num; ++wi) {
    ...
    for (int32_t ci = 0; ci < num_chunks; ++ci) {
...

      {
        ...
        GmTensor2D<ComputeT> s_global(workspace_handle + ws_base + WS_S, s_shape,
                                  s_stride);
...

      {
        ...
        GmTensor2D<ComputeT> ws_global(workspace_handle + ws_base + WS_WS,
                                   ws_shape, ws_stride);
...

      {
        ...
        GmTensor2D<ComputeT> k_global(workspace_handle + ws_base + WS_K, k_shape,
                                  k_stride);
      ...
      int64_t v_offset = ((chunk_start) * H + head) * D;
      {
        ...
        GmTensor2D<ComputeT> v_global(V_handle + v_offset, v_shape, v_stride);
...

      {
        ...
        GmTensor2D<ComputeT> kv_global(workspace_handle + ws_base + WS_KV,
                                   kv_shape, kv_stride);
  ...
  for (int64_t wi = 0; wi < (total_work + block_num - 1) / block_num; ++wi) {
    ...
    if (has_initial_state != 0) {
      ...
      GmTensor2D<ComputeT> h0_global(H0_handle + h0_offset, h0_shape, h0_stride);
    ...
    {
      // `workspace_handle` is a `ComputeT*`, so all offsets here are in ComputeT elements.
      GmShape2D s_shape(HalfC, D);
      GmStride2D s_stride(D);
      GmTensor2D<ComputeT> s_global(
          workspace_handle + ws_base + WS_S + vid * HalfC * D,
          s_shape, s_stride);
      DynVecTile<ComputeT, HalfC, D> s_store(HalfC, D);
    ...
    if (output_final_state != 0) {
      ...
      int64_t fs_offset = (seq_idx * H + head) * DD;
      {
        ...
        GmTensor2D<ComputeT> fs_global(FS_handle + fs_offset + vid * HalfC * D,
                                   fs_shape, fs_stride);
