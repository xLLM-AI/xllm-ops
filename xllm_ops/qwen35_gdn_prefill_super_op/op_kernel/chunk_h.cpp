...
AICORE void chunk_h_kernel(__gm__ half *K_handle, __gm__ half *W_handle, __gm__ half *U_handle, __gm__ float *G_handle,
                           ...
                           __gm__ float *FS_cache_handle, int64_t fs_cache_index)
{
    ...
    for (int64_t wi = 0; wi < (total_work + block_num - 1) / block_num; ++wi) {
        ...
        // Preserve MegaChunkGdn's FP16 final-state rounding point before the
        // persistent cache is materialized as FP32.
        pipe_barrier(PIPE_V);
        TCVT(s_ub, s_ub_half, pto::RoundMode::CAST_NONE);
        ...
        {
            int64_t cache_offset = (fs_cache_index * H + head) * DD + vid * HalfC * D;
            GmShape2D cache_shape(HalfC, D);
            GmStride2D cache_stride(D);
            GmTensor2D<float> cache_global(FS_cache_handle + cache_offset, cache_shape, cache_stride);
