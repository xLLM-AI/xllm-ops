...
#define CALL_XATTN_V2_KERNEL(INPUT_TYPE, SHARED_PAGED_FLAG, UNSHARED_PAGED_FLAG) \
    do { \
...
extern "C" __global__ __aicore__ void x_attention_v2(GM_ADDR query, GM_ADDR shared_key_block, GM_ADDR shared_value_block, 
    ...
    if (TILING_KEY_IS(4)) {          // 0b0100
        CALL_XATTN_V2_KERNEL(half, false, true);
    } else if (TILING_KEY_IS(6)) {   // 0b0110
        CALL_XATTN_V2_KERNEL(bfloat16_t, false, true);
    } else if (TILING_KEY_IS(8)) {   // 0b1000
        CALL_XATTN_V2_KERNEL(half, true, false);
    } else if (TILING_KEY_IS(10)) {  // 0b1010
        CALL_XATTN_V2_KERNEL(bfloat16_t, true, false);
    }
}
