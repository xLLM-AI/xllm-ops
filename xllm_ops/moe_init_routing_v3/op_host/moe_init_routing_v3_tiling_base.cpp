...
{   
    ...
    OPS_LOG_E_WITHOUT_REPORT(context, compileInfo);
    auto platformInfo = context->GetPlatformInfo();
    OPS_LOG_E_WITHOUT_REPORT(context, platformInfo);
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(platformInfo);
    compileInfo->aivNum = ascendcPlatform.GetCoreNumAiv();
    OPS_CHECK((compileInfo->aivNum <= 0), OPS_LOG_E(context, "TilingPrepareForMoeInitRountingV3 fail to get core num."),
    ...
    OPS_CHECK((compileInfo->ubSize <= 0), OPS_LOG_E(context, "TilingPrepareForMoeInitRountingV3 fail to get ub size."),
                 return ge::GRAPH_FAILED);
