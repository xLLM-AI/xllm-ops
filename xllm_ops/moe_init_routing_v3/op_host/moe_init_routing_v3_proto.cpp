                                            ...
                                            const int64_t expertEnd, const int64_t quantMode)
{
    ...
    if (QuantMode::NON_QUANT == quantMode) {
        if (scaleShape->GetDimNum() == DIM_ONE) {
            OPS_CHECK(scaleShape->GetDim(0) < 0 && scaleShape->GetDim(0) != NEG_ONE && scaleShape->GetDim(0) != NEG_TWO,
                        OPS_LOG_E(context,
                                "When quant_mode is %ld and use scale in dynamic graph, The shape of scale should be (-1) or (-2), current shape is (%s).",
                                quantMode, ToString(*scaleShape).c_str()),
                        return ge::GRAPH_FAILED);
            OPS_CHECK(scaleShape->GetDim(0) > 0 && !isSameDim(scaleShape->GetDim(0), xShape->GetDim(0)),
                        OPS_LOG_E(context,
                                "When quant_mode is %ld and use scale in static graph, The shape of scale should be (%ld,), current shape is (%s).",
                                quantMode, xShape->GetDim(0), ToString(*scaleShape).c_str()),
                        return ge::GRAPH_FAILED);
        } else {
        ...
        if (scaleShape->GetDimNum() == DIM_ONE) {
            OPS_CHECK(scaleShape->GetDim(0) != NEG_TWO,
                        OPS_LOG_E(context,
            ...
            if (scaleShape->GetDim(0) > 0) {
                ...
                OPS_CHECK(
                    ...
                    OPS_LOG_E(context,
                            ...
                            quantMode, xShape->GetDim(1), ToString(*scaleShape).c_str()),
                    return ge::GRAPH_FAILED);
            }
        } else {
            OPS_LOG_E(
                                       ...
                                       const int64_t expertEnd, const int64_t quantMode)
{
    ...
    if (expert_idx_n < NEG_ONE || expert_idx_k < NEG_ONE) {
        OPS_LOG_E(context, "Invalid expert_idx shape, shape is %s.", ToString(*expertIdxShape).c_str());
        return ge::GRAPH_FAILED;
    }
...
{
    OPS_LOG_D(context, "expanded_x shape is: %s after infershape.", ToString(*expandedXShape).c_str());
    ...
    OPS_LOG_D(context, "expanded_scale shape is: %s after infershape.", ToString(*expandedScaleShape).c_str());
}
...
static ge::graphStatus InferShape4MoeInitRoutingV3(gert::InferShapeContext *context)
{
    ...
    if (QuantMode::NON_QUANT == quantMode || QuantMode::DYNAMIC_QUANT == quantMode) {
        ...
        int64_t dim1 =
            (cols == NEG_ONE) ?
                NEG_ONE :
                CeilAlign<int64_t>(CeilDiv<int64_t>(cols, MX_QUANT_BLOCK_SIZE), 2LL);
...
static ge::graphStatus InferShapeRange4MoeInitRoutingV3(gert::InferShapeRangeContext *context)
{
    ...
    // OPS_LOG_D(context, "Before InferShapeRange, count->GetMin() = %s", ToString(*(count->GetMin())).c_str());
    // OPS_LOG_D(context, "Before InferShapeRange, count->GetMax() = %s", ToString(*(count->GetMax())).c_str());
    ...
    // OPS_LOG_D(context, "After InferShapeRange, count->GetMin() = %s", ToString(*(count->GetMin())).c_str());
    // OPS_LOG_D(context, "After InferShapeRange, count->GetMax() = %s", ToString(*(count->GetMax())).c_str());
