...
// CATLASS_ARCH=3510 so the arch35 host tiling struct/branch is compiled in; on A3
// it is empty. Included here (in the shared tiling header) so that EVERY
// translation unit that pulls in this header (tiling.cpp, proto.cpp, ...) sees a
// consistent XFAInferTilingData layout, avoiding ODR violations.
#include "xfa_arch_config.h"
...
BEGIN_TILING_DATA_DEF(XFAInferTilingData)
  ...
  // A5(arch35) path memcpy's the full 904B FATilingData into the RawTilingData
  // buffer. The framework sizes RawTilingData from this registered struct, so we
  // reserve a padding blob large enough to hold FATilingData. A3 builds do NOT
  // define CATLASS_ARCH, so this field is absent and the A3 layout is unchanged.
  TILING_DATA_FIELD_DEF_ARR(uint8_t, 1024, xfaA5TilingReserved);
#endif
END_TILING_DATA_DEF;
