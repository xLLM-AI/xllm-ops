...
function(gen_aclnn_master_header aclnn_master_header_name aclnn_master_header opbuild_out_headers)
  ...
  configure_file(
    "${CMAKE_CURRENT_SOURCE_DIR}/../cmake/aclnn_ops_xllm.h.in"
    "${aclnn_master_header}"
    @ONLY
  )
endfunction()
...
function(gen_aclnn_with_opdef)
  ...
  if(NOT ENABLE_BUILT_IN)
    set(aclnn_master_header_name "aclnn_ops_xllm_${VENDOR_NAME}")
  else()
    set(aclnn_master_header_name "aclnn_ops_xllm")
  endif()
  ...
  foreach(op_aclnn_name ${mc2_op_aclnn_name})
    if (NOT ENABLE_BUILT_IN AND NOT ("${ASCEND_OP_NAME}" STREQUAL "ALL"))
      foreach(op_name IN LISTS ASCEND_OP_NAME)
        file(GLOB matching_file "${OPS_XLLM_DIR}/mc2/${op_name}/op_api/aclnn_${op_aclnn_name}.h")
        list(APPEND mc2_aclnn_master_headers ${matching_file})
      endforeach()
    else()
      file(GLOB matching_file "${OPS_XLLM_DIR}/mc2/*/op_api/aclnn_${op_aclnn_name}.h")
      list(APPEND mc2_aclnn_master_headers ${matching_file})
    endif()
  endforeach()
...
function(merge_graph_headers)
  ...
  add_custom_command(OUTPUT ${MGPROTO_OUT_DIR}/ops_proto_xllm.h
    COMMAND ${ASCEND_PYTHON_EXECUTABLE} ${CMAKE_SOURCE_DIR}/../scripts/util/merge_proto.py
    ${proto_headers}
    --output-file ${MGPROTO_OUT_DIR}/ops_proto_xllm.h
  )

  add_custom_command(
    OUTPUT ${MGPROTO_OUT_DIR}/ops_proto_xllm.cpp
    COMMAND ${CMAKE_COMMAND} -E copy
      ${MGPROTO_OUT_DIR}/ops_proto_xllm.h
      ${MGPROTO_OUT_DIR}/ops_proto_xllm.cpp
    DEPENDS ${MGPROTO_OUT_DIR}/ops_proto_xllm.h
  )

  add_custom_target(${MGPROTO_TARGET} ALL
    DEPENDS ${MGPROTO_OUT_DIR}/ops_proto_xllm.h ${MGPROTO_OUT_DIR}/ops_proto_xllm.cpp
  )
endfunction()