...
check_opp_version_file() {
  ...
  # ops_xllm/version.info -> ops_xllm
  elif [ -f "${_DEFAULT_INSTALL_PATH}/ops_xllm/version.info" ]; then
    ver_info="${_DEFAULT_INSTALL_PATH}/ops_xllm/version.info"
  else
    logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The [ops_xllm] version.info not exists."
  fi
  return
}
...
show_relation() {
  ...
  if [ "$relation_situation" = "SUCC" ]; then
    logandprint "[INFO]: Relationship of ops_xllm with ${req_pkg_name_val} in path ${req_pkg_path} check successfully"
    return 0
  else
    logandprint "[WARNING]: Relationship of ops_xllm with ${req_pkg_name_val} in path ${req_pkg_path} check failed. \
