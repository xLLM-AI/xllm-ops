...
logandprint() {
    ...
    echo "[OpsXllm] [$(getdate)] ""$1" >> "${_INSTALL_LOG_FILE}"
}
...
if [ ! -f "${install_shell}" ]; then
    logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST};OpsXllm module is not installed or some ops_xllm source files are lost.\
