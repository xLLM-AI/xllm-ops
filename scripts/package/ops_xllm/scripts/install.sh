...
OPP_COMPATIBILITY_CEHCK_ERR_DES="OppXllm compatibility check error."
PERM_DENIED="0x0093"
PERM_DENIED_DES="Permission denied."

OPP_PLATFORM_DIR=ops_xllm
...
# 包内路径
GRAPH_SO_PATH="${CURR_PATH}/../../../../${OPP_PLATFORM_DIR}/built-in/op_graph/lib/linux/${ARCH_INFO}/libopgraph_xllm.so"
HOST_SO_PATH="${CURR_PATH}/../../../../${OPP_PLATFORM_DIR}/built-in/op_impl/ai_core/tbe/op_host/lib/linux/${ARCH_INFO}/libophost_xllm.so"
...
startlog() {
  echo "[OpsXllm] [$(getdate)] [INFO]: Start Time: $(getdate)"
}

exitlog() {
  echo "[OpsXllm] [$(getdate)] [INFO]: End Time: $(getdate)"
}
...
check_docker_path() {
  docker_path="$1"
  if [ "${docker_path}" != "/"* ]; then
    echo "[OpsXllm] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --docker-root\
 must with absolute path that which is start with root directory /. Such as --docker-root=/${docker_path}"
    exitlog
    exit 1
  fi
  if [ ! -d "${docker_path}" ]; then
    echo "[OpsXllm] [ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${docker_path} not exist, please create this directory."
    exitlog
    exit 1
  fi
}
...
judgment_path() {
  ...
  if [ $? -ne 0 ]; then
    echo "[OpsXllm][ERROR]: The opp install path ${1} is invalid, only characters in [a-z,A-Z,0-9,-,_] are supported!"
    exitlog
    exit 1
  fi
}
...
check_install_path() {
  ...
  if [ "x${TARGET_INSTALL_PATH}" = "x" ]; then
    echo "[OpsXllm] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Parameter --install-path\
 ...
 not support that the install path contains space character."
  ...
  if [ "x${prefix}" = "x" ]; then
    ...
    if [ x"${prefix}" = "x" ]; then
      ...
      echo "[OpsXllm] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES: Run package path is invalid: $RUN_PATH"
      exitlog
      exit 1
    fi
  fi
  ...
  while true; do
    ...
    case "$1" in
      ...
      --install-for-all)
        ...
        ;;
      -*)
        echo "[OpsXllm] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:Unsupported parameters [$1],\
...
check_opts() {
  if [ "${CONFLICT_CMD_NUMS}" != 1 ]; then
    echo "[OpsXllm] [ERROR]: ERR_NO:${PARAM_INVALID};ERR_DES:\
...
check_pre_install() {
  ...
  if [ "${IS_UPGRADE}" = "y" ]; then
    if [ ! -e "${INSTALL_INFO_FILE}" ]; then
      logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The directory:${TARGET_INSTALL_PATH} not install OpsXllm, upgrade failed."
      exitlog
      exit 1
    fi
...
install_package() {
  ...
  clean_before_reinstall
  if [ "$?" != 0 ]; then
    comm_log_operation "Install" "${IN_INSTALL_TYPE}" "OpsXllm" "$?" "${CMD_LIST}"
  fi
...
uninstall_package() {
 ...
 installed in (${TARGET_VERSION_DIR}) and then set the correct install path."
    ...
    comm_log_operation "Uninstall" "${IN_INSTALL_TYPE}" "OpsXllm" "$?" "${CMD_LIST}"
    exit 0
  fi
  ...
  local graph_so_path="${TARGET_VERSION_DIR}/opp/built-in/op_graph/lib/linux/${target_arch}/libopgraph_xllm.so"
  local graph_so_dir_path="${TARGET_VERSION_DIR}/opp/built-in/op_graph/lib/linux/${target_arch}"
  local host_so_path="${TARGET_VERSION_DIR}/opp/built-in/op_impl/ai_core/tbe/op_host/lib/linux/${target_arch}/libophost_xllm.so"
