...
def moe_init_routing_custom_golden(x, expert_idx, expert_num):
...
def test_moe_init_routing_custom(bs, h, k, expert_num, dtype):
    ...
    expanded_x_ref, expanded_row_idx_ref, count_ref = moe_init_routing_custom_golden(
        x_ref_np, expert_idx_np, expert_num
    )

    expanded_x, expanded_row_idx, count_or_cumsum, _scale = \
        custom_ops.moe_init_routing_custom_npu(
            x_t.npu(),
            expert_idx_t.npu(),
