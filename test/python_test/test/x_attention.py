...
class TestFlashAttentionInfer():
    ...
    class AttentionInputs:
        query: any # [bs, 1, headnum, headdim]
        key_cache: any  # [num_blocks, block_size, kv_head, headdim]
        value_cache: any
        unshared_k: any # [bs (request_num * beam_size), kv_head, max_decode_step, headdim]
        unshared_v: any # [bs (request_num * beam_size), kv_head, max_decode_step, headdim]
        block_tables: any # [bs, max_blocks_per_batch] max_blocks_per_bactch = ceil(max_shared_kvlen / block_size)
        q_seqlen_list: any # [request_num] (1, 1, 1)
        k_seqlen_list: any # [request_num] (share_len1, share_len2)
        global_mask: any
        mask_type: any
        shape_param: any
    ...
    def group_matmul(cls, head, kv_head, left, right, right_row=None, right_col=None):
        ...
        for i in range(kv_head):
            current_right = right[i:(i + 1), :, :] if right_row == None else right[i:(i + 1), :right_row, :right_col]
            group_score = np.matmul(left[i * group_num:(i + 1) * group_num, :, :].astype(np.float32),
                                    current_right.astype(np.float32))
            if score is None:
                score = group_score
            else:
    ...
    def ref_masked_attention(self,
        ...
        print("score shape:", sim_high.shape)
        # breakpoint()
        sim_high.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "s.bin"))
        # softmax
    ...
    def ref_single_query_unshared_kv_attention(self,
        ...
        for i in range(batch):
            ...
            unshared_gm[i, :, :] = gm[:, :, :]  # or gm.squeeze(-1) but keep last dim
            unshared_gl[i, :, :] = gl[:, :, :]
        #print("gm:", unshared_gm.reshape(-1, 32)[:3])
        #print("gl:", unshared_gl)

    def ref_single_query_shared_kv_attention(self, attention_inputs: AttentionInputs, output, true_out, 
    shared_gl, shared_gm) -> None:
        num_heads = attention_inputs.shape_param.num_heads
        kv_heads = attention_inputs.shape_param.kv_heads
        ...
        kv_seqlen_now = 0
        layout = 'TND'
        print("block_tables:", attention_inputs.block_tables)
        for i in range(request_num):
            ...
            cu_seqlen += q_seqlen
            kv_seqlen_now += kv_seqlen
            
    def call_device_op(self, q, k, v, unshared_k, unshared_v, block_tables, actual_shared_kvlen, decode_step):
        q = torch.from_numpy(copy.deepcopy(q)).npu()
        q = q.unsqueeze(1) # (bs*beam_size, 1, head_num, head_dim)
        k = torch.from_numpy(copy.deepcopy(k)).npu()
        v = torch.from_numpy(copy.deepcopy(v)).npu()
        unshared_k = torch.from_numpy(copy.deepcopy(unshared_k)).npu()
        unshared_v = torch.from_numpy(copy.deepcopy(unshared_v)).npu()
        ...
        attn_out = custom_ops.x_attention_npu(q, k, v, unshared_k, unshared_v, block_tables, actual_shared_kvlen, decode_step_tensor)
        return attn_out
    ...
    def calc_data(self, gen_data_params: GenDataParams):
        ...
        if gen_data_params.kv_dtype == 1:
            ...
            # unshared kv
            unshared_key = np.random.uniform(kv_min_range, kv_max_range,
            size=(batch_size, gen_data_params.kv_heads, max_decode_step, head_size_qk)).astype(gen_data_params.dtype)
            unshared_value = np.random.uniform(kv_min_range, kv_max_range,
            size=(batch_size, gen_data_params.kv_heads, max_decode_step, head_size_vo)).astype(gen_data_params.dtype)
        ...
        attention_inputs = self.AttentionInputs(query, key_cache, value_cache, unshared_key, unshared_value, 
        block_tables, gen_data_params.q_seqlen_list, gen_data_params.k_seqlen_list, mask, gen_data_params.mask_type, gen_data_params)
        ...
        # final_ref_out = (shared_ref_out * update_shared_expgm + unshared_ref_out * update_unshared_expgm) / gl

        # debug
        print("qTokens:", num_tokens)
        print("kv_ntokens", num_shared_kv)
        print("query:", query.shape)
        print("key_cache:", key_cache.shape)
        print("value_cache:", value_cache.shape)
        print("unshared_key:", unshared_key.shape)
        print("unshared_value:", unshared_value.shape)
        print("qSeqlen:", gen_data_params.q_seqlen_list)
        print("shareKvSeqlen:", gen_data_params.k_seqlen_list)
        print("block_tables:", block_tables)
        print("shared_golden:", shared_ref_out.shape)
        print("shared_gm:", shared_gm.shape)
        print("shared_gl:", shared_gl.shape)
        print("shared_o:", shared_true_out.shape)
        npu_res = self.call_device_op(query, key_cache, value_cache, unshared_key, unshared_value, block_tables, gen_data_params.k_seqlen_list, decode_step)
        golden_res = torch.from_numpy(final_true_out)
        golden_res = golden_res.unsqueeze(1)
        npu_res = npu_res.cpu().float()
        print("npu_res:", npu_res)
        print("golden_res:", golden_res)
        assert torch.allclose(npu_res, golden_res, atol=0.001, rtol=0.001)
        print(f"npu_res shape: {npu_res.shape}")
        # unshared_workspace_data = np.concatenate([unshared_true_out.ravel(), unshared_gm.ravel(), unshared_gl.ravel()])

        # unshared_workspace_data.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "unshared_workspace.bin"))
        # shared_workspace_data = np.concatenate([shared_true_out.ravel(), shared_gm.ravel(), 
        #                                          shared_gl.ravel()])
        # shared_workspace_data.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "shared_workspace.bin"))
        # #breakpoint()
        # num_tokens.astype(np.int32).tofile(os.path.join(WORKSPACE, "data", "q_ntokens.bin"))
        # num_shared_kv.astype(np.int32).tofile(os.path.join(WORKSPACE, "data", "kv_ntokens.bin"))
        # query.tofile(os.path.join(WORKSPACE, "data", "q.bin"))
        # key_cache.tofile(os.path.join(WORKSPACE, "data", "k_cache.bin"))
        # value_cache.tofile(os.path.join(WORKSPACE, "data", "v_cache.bin"))
        # unshared_key.tofile(os.path.join(WORKSPACE, "data", "unshared_k.bin"))
        # unshared_value.tofile(os.path.join(WORKSPACE, "data", "unshared_v.bin"))
        ...
        # final_true_out.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "golden.bin"))
        # shared_ref_out.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "shared_golden.bin"))
        # shared_gm.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "shared_gm.bin"))
        # shared_gl.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "shared_gl.bin"))
        # shared_true_out.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "shared_o.bin"))
        # unshared_ref_out.astype(np.float32).tofile(os.path.join(WORKSPACE, "data", "unshared_golden.bin"))
        # unshared_gm.tofile(os.path.join(WORKSPACE, "data", "unshared_gm.bin"))
        # unshared_gl.tofile(os.path.join(WORKSPACE, "data", "unshared_gl.bin"))

if __name__ == "__main__":
    os.makedirs(os.path.join(WORKSPACE, "data"), exist_ok=True)
    # usage python3 js_attention.py
    request = int(sys.argv[1]) # == batch
    beam_size = int(sys.argv[2])
    q_seqlen = int(sys.argv[3]) # must be 1 
    kv_seqlen = int(sys.argv[4]) # shared_kv_len
    unshared_seqlen = int(sys.argv[5])
    num_head = int(sys.argv[6])
    kv_heads = int(sys.argv[7])
    ...
    gen_data_params = testObj.GenDataParams(q_seqlen_list, kv_seqlen_list, beam_size, unshared_seqlen,
     num_head, kv_heads, embedding_size, num_blocks, block_size, mask_type, dtype, kv_dtype)
