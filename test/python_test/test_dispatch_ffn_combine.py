...
# -*- coding: utf-8 -*-
"""
pytest test for dispatch_ffn_combine (multi-card MoE dispatch+FFN+combine fusion op).
Migrated from vllm-ascend multicard_ops_a3.

Dependencies:
  - custom_ops_lib (pybind11 module built from RegisterOps.cpp)
  - HCCL multi-card (world_size=2)

NOTE: This test requires:
  1. custom_ops_lib.so compiled with aclnnDispatchFFNCombine support
...
class TestDispatchFFNCombine:
    ...
    def run_tensor_list(self) -> bool:
        ...
        xactmask = torch.randint(0, 2, (m,), dtype=torch.bool).npu()
        ...
        custom_ops_lib.dispatch_ffn_combine(
            x, weight1_nz_npu, weight2_nz_npu, expert_idx,
            scale1_npu, scale2_npu, probs,
            self.hcomm_info, 512, out, expert_token_nums,
            xactmask, 0.0,
        )
        return True
...
if __name__ == "__main__":
    test_dispatch_ffn_combine_kernel()