...
class CompileKernel:
    ...
    def ascendc_put_json(self: any):
        if self.json_file is not None:
            json_file_dir = os.path.join(self.working_dir,
                                         "customize",
                                         "op_impl",
                                         "ai_core",
                                         "tbe",
    ...
    def ascendc_build(self: any):
        ...
        with os.fdopen(os.open(mkfile, const_var.WFLAGS, const_var.WMODES), "w") as fd:
            ...
            fd.write("\n".join(sub_cmd))
        if os.system('export HI_PYTHON=python3 && export ASCEND_CUSTOM_OPP_PATH={} && export TILINGKEY_PAR_COMPILE=1'
                     '&& make -f {} PY={} OUT={} CPP={}'.format(self.build_opp_path, mkfile, self.op_impl_py,
                                                               op_bin_dir, self.op_cpp_file)):
            raise RuntimeError('Kernel Compile Error: OpType {} Kernel File {}!'.format(self.op_type, self.op_cpp_file))
...
def args_parse():
    ...
    parser.add_argument(
        "-b", "--build-tool", nargs="?", default=None, help="build tool mast be make."
    )
