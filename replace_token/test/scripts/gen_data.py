...
def gen_data(shape,forked_token_file_path,last_step_out_put_token_file_path,output_out_file_path):
    m = shape[0]
    forked_token = np.random.randint(-(m-1),m-1,shape,dtype=np.int32)
    forked_token.tofile(forked_token_file_path)
    last_step_out_put_token = np.random.randint(1,m-1,shape,dtype=np.int64)
...
if __name__ == "__main__":
    m=int(sys.argv[1])
    gen_data((m,1),"./input/forked_token_ids.bin","./input/last_step_out_put_token_ids.bin","./input/output_out.bin")
